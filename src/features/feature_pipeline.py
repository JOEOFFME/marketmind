...
def compute_proximity_features(
    places: pd.DataFrame, pois: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    for _, place in places.iterrows():
        ...
        def min_dist(subset):
            if subset.empty:
                return 9999.0
            dists = haversine(
                lat, lon, subset["latitude"].values, subset["longitude"].values
            )
...
def compute_competition_features(places: pd.DataFrame) -> pd.DataFrame:
    ...
    type_counts = (
        places.groupby(["district", "place_type"])
        .size()
        .reset_index(name="type_count_in_district")
    )
    places = places.merge(type_counts, on=["district", "place_type"], how="left")

    # Is this place above average for its type in its district?
    type_avg = (
        places.groupby(["district", "place_type"])["success_score"]
        .mean()
        .reset_index(name="type_avg_score")
    )
    places = places.merge(type_avg, on=["district", "place_type"], how="left")
    places["above_type_avg"] = (
        places["success_score"] > places["type_avg_score"]
    ).astype(int)
